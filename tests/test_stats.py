...
class StatMixin(ClientTest):
    ...
    def test___call__(self, client, stat, expected_stat_name_with_prefix):
        ...
        if self.stat_class not in BASE_STATS:
            stat(1)
            self.assertMockCalledWith(getattr(client, stat._function), expected_stat_name_with_prefix, 1, sample_rate=self.sample_rate)
    ...
    def test_apply(self, client, stat, expected_stat_name_with_prefix):
        ...
        if self.stat_class not in BASE_STATS:
            stat.apply(42)
            self.assertMockCalledWith(getattr(client, stat._function), expected_stat_name_with_prefix, 42, sample_rate=self.sample_rate)
...
class TestTimerStat(StatMixin):
    ...
    def test_contextmanager(self, client, stat):
        with self.check_time(client), stat.time():
            pass
...
class StatMixins(ClientTest):
    ...
    def stats(self, client):
        return Stats("prefix", Meter("m", "mdoc"), client=client)
